...
import { produce } from "immer"; // immer 라이브러리
  ...
  const [ user, setUser ] = useState({
    ...
    const handleAddressChange = (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      */
      ...
      // immer 라이브러리 사용
      // user를 복사한 새로운 객체(dtraft)를 만들어서 콜백 함수의 인자로 전달
      // 원본과 draft의 수정 사항을 비교해서 참조하는 상위 객체로 올라가면서 바꿈(이걸 immer가 해준다.)
      const newUser = produce(user, (draft) => {
        const address = draft.extra.addressBook.find(address => address.id === Number(event.target.id));
        address!.value = event.target.value;
      });

      // 객체 불변성 = 처음 가졌던 값 그대로 가져야 한다는 의미
