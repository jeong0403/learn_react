...
axios.defaults.timeout = 5000; // 서버에 어떠한 문제가 있어 특정 시간이 넘어가면 실패로 간주하도록 기본 셋팅을 제공해주기 때문에 작성 가능함

// fetch 함수 호출시 필요한 파라미터 타입 정의
interface FetchParams {
  url: string;
}
// 제네릭으로 정의한 이유: 타입을 더 이상 알 필요가 없기 때문
function useAxios<T>(fetchParams: FetchParams) {
  ...
  const requestFetch = async (params: FetchParams) => {
    ...
    try {
      ...
      // 서버에서 받은 데이터가 오류인지 등을 미리 axios가 내려주기 때문에 Todo상태 업데이트 응답 관련 코드 지워도 됨
        setData(res.data);
      } catch (err) {
      // 네트워크 오류 등 예외 발생시 처리
