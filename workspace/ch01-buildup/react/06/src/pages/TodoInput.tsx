...
function TodoInput({ addItem }: TodoInputProps) {
  ...
  // 사용자 입력을 다루는 경우에는 대부분 state로 관리(value 속성을 추가, onChange 이벤트로 value 수정 - 제어 컴포넌트)
  ...
  const handleAddKeydown = (event: KeyboardEvent) => {
    ...
    if (event.nativeEvent.isComposing) return; // 글자가 완성되지 않았을 경우 무시한다. (맥에서 엔터키로 글자 2번씩 추가 되는 오류 때문에 추가함)
